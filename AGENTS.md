...
- **[2026-02-11] 改动后强制重新部署**：以后任何代码改动完成后，必须在服务器 `154.201.95.70` 重新部署并做可访问性验证。
  - Why：用户明确要求每次改动都上线同步，避免“本地已修复、线上未生效”。
  - Impact：后续所有开发任务默认包含“部署到服务器 + 线上验收”步骤。

- **[2026-02-11] 登录流程改为独立页面**：未登录访问 `/`、`/index.html`、`/settings.html` 默认重定向到 `/login.html`。
  - Why：登录前页面应保持空白，仅展示独立登录页。
  - Impact：主页不再使用登录弹窗；账号管理入口统一放到设置区域。
